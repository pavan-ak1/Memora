...
export const getContents = async (req: Request, res: Response) => {
  try {
    ...
    res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ message: "Failed to fetch contents", error: error.message });
  }
};
...
try {
    ...
    const contentToDelete = await ContentModel.findOne({ _id: contentId, user: userId });

    if (!contentToDelete) {
      return res.status(StatusCodes.NOT_FOUND).json({ message: "Content not found or not owned by user." });
    }
    ...
    res.status(StatusCodes.OK).json({ message: `Content "${contentTitle}" deleted successfully.` });

  } catch (error: any) {
    console.error("Error deleting content:", error);
    res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ message: "Failed to delete content", error: error.message });
  }
}
